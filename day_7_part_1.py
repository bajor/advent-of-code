...
five_of_kind = sorted(five_of_kind, key=lambda x: x[0], reverse=True)
five_of_kind = [x[-1] for x in five_of_kind]
result.extend(five_of_kind)

four_of_kind = [(rank_card(x[0][0]), rank_card(x[0][1]), x[1]) for x in four_of_kind]
four_of_kind = sorted(four_of_kind, key=lambda x: (x[0], x[1]), reverse=True)
four_of_kind = [x[-1] for x in four_of_kind]
result.extend(four_of_kind)
...
three = sorted(three, key=lambda x: (x[0], x[1], x[2]), reverse=True)
...
two_pairs = sorted(two_pairs, key=lambda x: (x[0], x[1], x[2]), reverse=True)
two_pairs = [x[-1] for x in two_pairs]
result.extend(two_pairs)
...
one_pair = sorted(one_pair, key=lambda x: (x[0], x[1], x[2], x[3]), reverse=True)
one_pair = [x[-1] for x in one_pair]
result.extend(one_pair)
...
high_card = sorted(high_card, key=lambda x: (x[0], x[1], x[2], x[3], x[4]), reverse=True)
high_card = [x[-1] for x in high_card]
result.extend(high_card)
